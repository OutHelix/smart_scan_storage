...
В `docker-compose.yml` подключен именованный том `backend_data`. В нем сохраняются:
...
Папка `train/` добавлена в репозиторий отдельно, потому что обученные модели и артефакты обучения слишком большие для хранения и push в Git-репозиторий. В `train/` находятся материалы и зависимости для обучения и подготовки моделей, а сами крупные веса моделей не хранятся в Git.
